<template>
    <div>
        ...
        <Modal v-model="returnModal1" title='审核' @on-ok="returnItem(-1)" @on-cancel="returncancel1(false)">
            <p style="text-align:center;font-size:16px;">是否拒绝任务----<span style="color:red;">{{deleteName}}</span>----的审核</p>
        </Modal>
        <Modal v-model="editModal" title="任务资料" :mask-closable="false" footer-hide>
                ...
                <FormItem prop="share_thumb" class="formItem" v-if="formInline.type===3">
                    <row class="formRow">
                        ...
                        <i-col span='18'>
                            ...
                            <img :src="formInline.share_thumb===''?'':formInline.share_thumb" width="150px"
...
export default {
        ...
        return {
            ...
            column: [
                ...
                },
                {
                    ...
                    render:(h,params)=> {
                        return h('p',
                            params.row.verify_status===-1?'未通过':(params.row.verify_status===0?'待审核':'审核通过')
                        )
                    },
                },
                {
                                ...
                                "Button",
                                {
                                    ...
                                    nativeOn: {
                                        click: () => {
                                            ...
                                            this.formInline.total_price = params.row.total_price
                                            this.formInline.merchant_id = params.row.merchant_id
                                            this.formInline.title = params.row.title
                                            this.formInline.type = params.row.type
                                            this.formInline.num = params.row.num
                                            this.formInline.start_time = params.row.start_time
                                            this.formInline.time_limit = params.row.time_limit
                                            this.formInline.images = params.row.images
                                            this.formInline.qrcode_url = params.row.qrcode_url
                                            this.formInline.wx_content = params.row.wx_content
                                            this.formInline.dy_request = params.row.dy_request
                                            this.formInline.url = params.row.url
                                            this.formInline.comment = params.row.comment
                                            this.formInline.tt_request = params.row.tt_request
                                            this.formInline.share_price = params.row.share_price
                                            this.formInline.share_thumb = params.row.share_thumb
                                            this.formInline.msg = params.row.share_content
    ...
    methods: {
        returnItem(i){
            axios.request({
                url:'task/tasks/'+this.currentId+'/verify?verify_status='+i,
                method:'get'
            }).then(res=>{
                if(i===1){

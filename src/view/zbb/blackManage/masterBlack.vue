<template>
    <div>
        ...
        <Modal v-model="cancelModal" title='删除' @on-ok="cancelItem()" @on-cancel="cancelcancel(false)">
            <p style="text-align:center;font-size:16px;">是否删除----<span style="color:red;">{{deleteName}}</span>----的拉黑记录</p>
        </Modal>
    </div>
</template>
    ...
    methods: {
        returnExcel() {
            ...
            axios.request({
                ...
                token = res.data
                let url =
                "http://47.101.217.238/zbb/public/backend/masters/blacklist/export" + "?token="+token;
                window.open(url);
            })
        ...
        getList() {
            axios
                .request({
                    url: "masters/blacklist?page=" + this.currentPage,
                    method: "get"
                })
                .then(res => {
